            ...
            <div class="panel panel-info">
                ...
                <div class="panel-body">
					...
					<table class="table table-striped table-bordered text-center">
                		...
                		<tbody>
                			<tr v-for="country in countrys">
                				<td><router-link :to="`/country/form/${+country.code}`">{{country.code}}</router-link></td>
                				<td><router-link :to="`/country/form/${+country.code}`">{{country.name}}</router-link></td>
                				<td>
                					<a @click="showModal = true; editCountry(country)"><span class="glyphicon glyphicon-trash"></span></a>
    ...
    export default {
        ...
        data() {
            ...
            methods: {
                ...
                fetchData: function () {
					{
                        http.get(config.urlApi+'/country').then(response => {
                            if(response.body[0]) {
                                this.countrys = response.body;
                            }
                        }, response => {
